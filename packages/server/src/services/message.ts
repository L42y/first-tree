...
function stripUntrustedMetadataKeys(
  ...
  if (CONTEXT_TREE_RECOVERY_METADATA_KEY in meta && !options.allowContextTreeRecovery) {
    throw new BadRequestError(
      `Metadata key "${CONTEXT_TREE_RECOVERY_METADATA_KEY}" is reserved for server-authored Context Tree recovery messages.`,
      { code: "CONTEXT_TREE_RECOVERY_METADATA_RESERVED" },
    );
  }
  if (SERVER_AUTHORED_METADATA_KEY in meta) {
    throw new BadRequestError(
      `Metadata key "${SERVER_AUTHORED_METADATA_KEY}" is derived by the server and cannot be supplied by callers.`,
      { code: "SERVER_AUTHORED_METADATA_RESERVED" },
    );
  }
...
export type SendMessageOptions = {
  ...
  /**
   * Trusted-internal capability for persisting a Context Tree recovery
   * fingerprint. These diagnoses are generated by the server but deliberately
   * delivered in the owning human agent's voice.
   */
  allowContextTreeRecovery?: boolean;
  /**
   * Marks a message as authored by a trusted server path even when it is
   * persisted with a human sender. This option is internal-only and the
   * corresponding metadata key is always rejected from caller input.
   *
   * Other trusted server capabilities (`allowSystemSender`,
   * `allowContextReviewRun`, `allowCronTrigger`, and
   * `allowContextTreeRecovery`) imply this marker automatically.
   */
  markServerAuthored?: boolean;
  /**
...
export function preflightMessageSendIntent(input: {
  ...
  const isServerAuthored =
    options.markServerAuthored === true ||
    options.allowSystemSender === true ||
    options.allowContextReviewRun === true ||
    options.allowCronTrigger === true ||
    options.allowContextTreeRecovery === true;
  const metadataToStore: Record<string, unknown> = {
    ...incomingMeta,
    ...(isServerAuthored ? { [SERVER_AUTHORED_METADATA_KEY]: true } : {}),
