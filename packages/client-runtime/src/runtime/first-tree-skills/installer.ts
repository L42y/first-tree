...
/**
 * Walk up from the running module to find the @first-tree/client-runtime package root,
...
export function resolveBundledSkillsRoot(startDir?: string): string {
  ...
  while (true) {
    ...
    if (parentDir === currentDir) {
      throw new Error(
        "Could not locate bundled `skills/` payloads. Run `pnpm --filter @first-tree/client-runtime prebuild` or check that the npm tarball includes `skills/`.",
      );
    }
