...
| `managed-unresolved-binding-work` | A managed workspace whose briefing states the Context Tree binding could not be confirmed, while the last-known `.first-tree/workspace.json` manifest and `context-tree/` checkout remain on disk, receives an ordinary software task followed by an explicit Tree read/write request. | The ordinary task continues from local inputs with zero Tree CLI invocations, zero reads or references of the stale manifest/checkout, and silence about the binding state. The explicit Tree request states only that this specific Tree operation cannot be completed right now because the binding could not be confirmed — never claiming that no Tree is bound, never guessing a Tree from the stale artifacts, and offering no bind/create/setup steering. | Transcript plus shim events showing zero Tree CLI invocations and no tool reads of `.first-tree/workspace.json` or `context-tree/` paths. |
...
| `forge-cli-absent-explicit-forge-op` | With no `gh`/`glab` on PATH, the user explicitly requests a forge operation (open a PR/MR, comment on an issue, read checks or provider metadata). | Only that forge step is blocked: the agent names the blocked step, gives the single narrowest recovery (install/auth the matching CLI, or hands back a local diff/instructions), and continues every independent part of the task. It never declares the whole task or workspace unusable. | Transcript showing the narrow blocked-step statement, the one recovery step, and the independent work still completed. |
...
- A missing capability (no bound Tree, no `gh`/`glab`) removes only the
  ...
  continuing. An unconfirmed (unresolved) Tree binding degrades the same way,
  but the agent never claims that no Tree is bound and never reads or trusts
  the last-known manifest or Tree checkout left on disk.
