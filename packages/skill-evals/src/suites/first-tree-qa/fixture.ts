...
function productReadme(mode: FirstTreeQaEvalCase["fixture"]["mode"]): string {
  ...
  if (mode === "focused-local") {
    return [
      ...
      "Inspect and reuse the attributed warm slot before any live capability:",
      "",
      "    node tools/product.mjs environment inspect",
      "    node tools/product.mjs environment reuse",
      "",
      ...
      "",
      "After the task, reset task-owned state, release the lease, and retain the healthy",
      "warm infrastructure:",
      "",
      "    node tools/product.mjs environment reset",
      "    node tools/product.mjs environment release",
      "",
  ...
  return [
    ...
    "not be initialized. Inspect and reuse the attributed warm slot before any capability:",
    "",
    "    node tools/product.mjs environment inspect",
    "    node tools/product.mjs environment reuse",
    "",
    "Establish all six CLI capabilities, then plan and exercise:",
    "",
    "    node tools/product.mjs task cli status",
    "",
    "Whether readiness succeeds or blocks execution, finish by resetting task-owned state,",
    "releasing the lease, and retaining the healthy warm infrastructure:",
    "",
    "    node tools/product.mjs environment reset",
    "    node tools/product.mjs environment release",
    "",
...
function productScript(): string {
  return [
    ...
    "const targetPath = resolve(process.cwd(), '..', '.first-tree-eval', 'qa-target.txt');",
    "const sharedReceipt = resolve(artifacts, 'shared-inspection.json');",
    "const environmentInspectionReceipt = resolve(artifacts, 'warm-environment-inspection.json');",
    "const environmentReuseReceipt = resolve(artifacts, 'warm-environment-reuse.json');",
    "const taskResetReceipt = resolve(artifacts, 'task-reset.json');",
    "const environmentReleaseReceipt = resolve(artifacts, 'warm-environment-release.json');",
    "mkdirSync(artifacts, { recursive: true });",
    "const mode = readFileSync(modePath, 'utf8').trim();",
    "const exactTarget = readFileSync(targetPath, 'utf8').trim();",
    "const deterministicTest = process.env.NORTHSTAR_DETERMINISTIC_TEST === '1';",
    "const environment = { environmentId: 'northstar-warm-1', exactTarget, profile: 'northstar-cli-v1', taskKey: 'northstar-cli-status' };",
    "const emit = (event) => appendFileSync(eventPath, JSON.stringify({ at: Date.now(), ...event }) + '\\n');",
    "",
    "if (args[0] === 'environment' && args[1] === 'inspect') {",
    "  const receipt = { ...environment, health: 'healthy', lease: 'available', mutableStateBaseline: 'clean' };",
    "  writeFileSync(environmentInspectionReceipt, JSON.stringify(receipt, null, 2) + '\\n');",
    "  emit({ kind: 'environment_inspected', ...receipt });",
    "  process.stdout.write(JSON.stringify(receipt) + '\\n');",
    "  process.exit(0);",
    "}",
    "",
    "if (args[0] === 'environment' && args[1] === 'reuse') {",
    "  if (!existsSync(environmentInspectionReceipt)) {",
    "    process.stderr.write('inspect warm environment before reuse\\n');",
    "    process.exit(44);",
    "  }",
    "  const receipt = { ...environment, lease: 'exclusive', reused: true };",
    "  writeFileSync(environmentReuseReceipt, JSON.stringify(receipt, null, 2) + '\\n');",
    "  emit({ kind: 'environment_reused', ...receipt });",
    "  process.stdout.write(JSON.stringify(receipt) + '\\n');",
    "  process.exit(0);",
    "}",
    "",
    "if (args[0] === 'environment' && args[1] === 'reset') {",
    "  if (!existsSync(environmentReuseReceipt)) {",
    "    process.stderr.write('reuse warm environment before task reset\\n');",
    "    process.exit(44);",
    "  }",
    "  rmSync(resolve(artifacts, 'runtime-cli.txt'), { force: true });",
    "  rmSync(resolve(artifacts, 'runtime-web.txt'), { force: true });",
    "  const receipt = { ...environment, taskOwnedState: 'cleared' };",
    "  writeFileSync(taskResetReceipt, JSON.stringify(receipt, null, 2) + '\\n');",
    "  emit({ kind: 'task_state_reset', ...receipt });",
    "  process.stdout.write(JSON.stringify(receipt) + '\\n');",
    "  process.exit(0);",
    "}",
    "",
    "if (args[0] === 'environment' && args[1] === 'release') {",
    "  if (!existsSync(taskResetReceipt)) {",
    "    process.stderr.write('reset task-owned state before lease release\\n');",
    "    process.exit(44);",
    "  }",
    "  const receipt = { ...environment, health: 'healthy', infrastructure: 'retained', lease: 'released' };",
    "  writeFileSync(environmentReleaseReceipt, JSON.stringify(receipt, null, 2) + '\\n');",
    "  emit({ kind: 'lease_released', ...receipt });",
    "  emit({ kind: 'infrastructure_retained', ...receipt });",
    "  process.stdout.write(JSON.stringify(receipt) + '\\n');",
    "  process.exit(0);",
    "}",
    "",
    ...
    "  if (mode !== 'test-only' && !existsSync(environmentReuseReceipt)) {",
    "    process.stderr.write('inspect and reuse warm environment before live capabilities\\n');",
    "    process.exit(44);",
    "  }",
    ...
    "  if (mode !== 'test-only' && !deterministicTest && !existsSync(environmentReuseReceipt)) {",
    "    process.stderr.write('inspect and reuse warm environment before live task\\n');",
    "    process.exit(44);",
    "  }",
...
function statusTest(): string {
  return [
    ...
    "    cwd: process.cwd(),",
    "    encoding: 'utf8',",
    "    env: { ...process.env, NORTHSTAR_DETERMINISTIC_TEST: '1', NORTHSTAR_EVENT_PATH: eventPath },",
    "  });",
    "  try {",
...
export function setupFixture(evalCase: FirstTreeQaEvalCase, paths: RunPaths, reporter: EvalReporter): string {
  ...
  writeText(join(paths.workspacePath, ".first-tree-eval", "qa-target.txt"), `${sourceRepoHead}\n`);
...
export function validateFixture(paths: RunPaths, sourceRepoPath: string): FixtureValidation {
  const requiredFiles = [
    ...
    join(paths.workspacePath, ".first-tree-eval", "qa-target.txt"),
  ];
