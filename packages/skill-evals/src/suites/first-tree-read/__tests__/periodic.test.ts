...
describe("first-tree-read periodic cases", () => {
  it("declares the runtime-generated briefing and unbound continuation rows as the implemented periodic set", () => {
  ...
  it("builds the managed-unbound runtime briefing without a manifest or tree checkout", () => {
    const evalCase = findFirstTreeReadPeriodicCase("first-tree-read-unbound-software-continues-periodic");
    if (evalCase === null) throw new Error("missing unbound read periodic case");

    const packageRoot = resolve(dirname(fileURLToPath(import.meta.url)), "../../../..");
    const paths = createRunPaths({
      caseId: "read-unbound-briefing-fixture-test",
      packageRoot,
      startedAt: new Date().toISOString(),
    });
    ...
    try {
      ...
      // The real runtime writes no workspace manifest when no Tree is bound.
  ...
  it("builds the stale-checkout briefing with the stale manifest and retired Tree checkout present", () => {
    ...
    try {
      ...
      // Explicit unbind deletes nothing: the stale manifest and the retired
      // checkout stay on disk as inert residue; the briefing must still say
      // explicitly unbound.
