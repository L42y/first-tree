...
import type { CaseRunSummary, CliOptions, ReturnMeetingContextEvalCase } from "./types.js";

export async function runReturnMeetingContextCase(
  packageRoot: string,
  ...
  const validatorResult = runCommand(
    process.execPath,
    [
      join(paths.workspacePath, ".agents", "skills", "return-meeting-context", "scripts", "validate-output.mjs"),
      "--bundle",
      join(sourceRepoPath, "bundle.json"),
      "--output",
      join(paths.workspacePath, "meeting-context-output.json"),
    ],
    paths.workspacePath,
  );
  ...
  const grading = buildGrading(evalCase, fixtureValidation, metrics);
