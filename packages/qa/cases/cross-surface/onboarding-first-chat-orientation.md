...
7. Start a fresh kickoff with the capability omitted. It retains the legacy immediate wake. Verify campaign action,
   dedicated tree setup, and ordinary chat messages never render or accept the trusted Orientation marker.
8. At a narrow phone viewport, the initial chapter choice plus global skip are visible without traversing an empty video
...
legacy reconciliation, and campaign/tree/ordinary-chat exclusion. `FAIL` includes an early or duplicate wake, lost
bootstrap context, inaccessible skip, ownership-misleading security copy, or a marker accepted from an untrusted send.
