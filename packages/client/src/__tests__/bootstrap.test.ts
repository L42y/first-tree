...
describe("inline skill installer — copy/version/symlink mechanics", () => {
  ...
  it("reinstalls only the skills whose VERSION drifted", () => {
    ...
    const bundledMixed = makeFixtureSkillsRoot("drift-mixed", [
      ...
      { name: "first-tree-qa", version: "1.0.0" },
      { name: "first-tree-read", version: "1.0.0" },
      { name: "first-tree-write", version: "1.0.0" },
      { name: "context-tree-review", version: "1.0.0" },
      { name: "context-tree-audit", version: "1.0.0" },
      { name: "return-meeting-context", version: "1.0.0" },
    ]);
  ...
  it("returns false when a bundled skill source is missing", () => {
    ...
    expect(existsSync(join(workspace, ".agents", "skills", "first-tree-qa"))).toBe(false);
    ...
    expect(existsSync(join(workspace, ".agents", "skills", "context-tree-audit"))).toBe(false);
    expect(existsSync(join(workspace, ".agents", "skills", "return-meeting-context"))).toBe(false);
    expect(logs.join("\n")).toContain(
      "failed first-tree-welcome, first-tree-file-bug, first-tree-qa, first-tree-read, first-tree-write, context-tree-review, context-tree-audit, return-meeting-context",
    );
