...
describe("renderRuntimeOutputContract", () => {
  ...
  it("classifies reply transport apart from business actions (hold-off carve-out)", () => {
    ...
    expect(contract).toMatch(/explicit request to archive the current chat/i);
    expect(contract).toMatch(/send the required final human reply first/i);
    expect(contract).toMatch(/run `chat archive` as the last business action/i);
