...
async function downloadExactArtifact(
  ...
  const response = await fetchImpl(contract.artifactUrl, { redirect: "error" });
  try {
    if (!response.ok) {
      throw new Error(`official ZCode download returned HTTP ${response.status}`);
    }
    const declaredLength = Number(response.headers.get("content-length"));
    if (response.headers.has("content-length") && (!Number.isSafeInteger(declaredLength) || declaredLength < 0)) {
      throw new Error("official ZCode download has a malformed Content-Length");
    }
    if (response.headers.has("content-length") && declaredLength !== contract.artifact.bytes) {
      throw new Error(`official ZCode download size mismatch: HTTP declared ${declaredLength} bytes`);
    }
    if (!response.body) throw new Error("official ZCode download has no body");
    ...
    try {
      ...
      for await (const chunk of body) {
        ...
        if (bytes > contract.artifact.bytes) {
          throw new Error(`official ZCode download exceeds ${contract.artifact.bytes} bytes`);
        }
        await output.write(chunk);
      }
      if (bytes !== contract.artifact.bytes) {
        throw new Error(`official ZCode download size mismatch: received ${bytes} bytes`);
      }
      await output.sync();
    } finally {
      await output.close();
