...
| `create<Provider>AuthDriver` | Resolve the artifact, spawn the official login, re-probe affected rows | provider family `login.ts` (Claude: `providers/claude/login.ts`, Grok: `providers/grok/login.ts`); others still transitional under `runtime/<provider>-login.ts` |
