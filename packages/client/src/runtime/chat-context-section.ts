...
export function renderRuntimeOutputContract(): string {
  ...
  return [
    ...
    `The console addresses the runtime; running one of these commands is what places your message in front of a teammate. Describing a reply in your output records words on the console, while running the command delivers them. Even for a simple question or greeting, answering a teammate is two acts, not one: do the work (console), then deliver the result with \`${bin} chat send\` (outbox).`,
    "",
