...
import type { HandlerConfig } from "@first-tree/client-runtime/runtime/handler.js";
import { providerSkillRoot } from "@first-tree/client-runtime/runtime/managed-skills.js";
...
describe("builtin handler registry", () => {
  ...
  it("composition root is createBuiltinHandlerRegistry only — no process-global install path", async () => {
    ...
    const runtimeSrc = resolve(fileURLToPath(new URL("../../../client-runtime/src/runtime", import.meta.url)));
    const handlerSource = readFileSync(resolve(runtimeSrc, "handler.ts"), "utf8");
    ...
    const runtimeSource = readFileSync(resolve(runtimeSrc, "runtime.ts"), "utf8");
