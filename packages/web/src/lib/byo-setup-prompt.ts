...
export function buildByoSetupPrompt({
  ...
  return [
    ...
    "",
    "7. When setup is complete, verify the current-session handoff uses schema version 3, then adopt its `activationContext`, loader commands, provider, and immutable project identity as the immutable activation receipt. Keep that provider/project identity even if cwd changes later. For every new Context task, use the loader again instead of reusing a prior Core Skill path.",
    "",
