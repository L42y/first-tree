  ...
  row({
    ...
    payload: {
      ...
      body: [
        ...
        "## Core principle",
        "",
        "packages/web is a token-based design system: every visual value comes from a CSS variable defined in `index.css` — never a literal. The token lint is a build gate; hardcoded values fail the build. Read `DESIGN.md` before writing any UI code, and follow it exactly.",
        "",
        ...
        "- **Use the semantic layer only.** Use the foreground, background, state, and radius tokens. Do not reach for raw Tailwind palette or size utilities.",
        "- **New base components** go in `components/ui/`, composed with `cva` + `cn` + Radix.",
        "",
        "## Before you claim done",
        "",
        "Run the web typecheck (tsc plus the token lint) and confirm it passes. A token violation fails the build — green is the only definition of done.",
        "",
        "## Common rationalizations — all wrong",
        "",
        "| Excuse | Reality |",
        "|--------|---------|",
        "| 'Just one literal, I'll tokenize later' | The token lint fails the build now. Add the token now. |",
        "| 'There's no token for this value' | Add one to `index.css`. That's the workflow, not an exception. |",
        "| 'A raw palette class is close enough' | Raw palette utilities are banned. Use the semantic token. |",
      ].join("\n"),
