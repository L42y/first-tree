...
export function QuickActions({ visible, onClose }: { visible: boolean; onClose: () => void }) {
  ...
  const items = useMemo<Item[]>(() => {
    ...
    for (const agent of agentsQuery.data ?? []) {
      ...
      out.push({ key: `a-${agent.uuid}`, section: "Agents", label: agent.displayName, sub: agent.type, action: () => go(`/agent/${agent.uuid}`) });
    }
    for (const chat of chatsQuery.data ?? []) {
      out.push({ key: `h-${chat.chatId}`, section: "Chats", label: chat.title, sub: chat.lastMessagePreview ?? undefined, action: () => go(`/chat/${chat.chatId}`) });
    }
    for (const doc of docsQuery.data?.items ?? []) {
      out.push({ key: `d-${doc.id}`, section: "Docs", label: doc.title, sub: doc.project ?? undefined, action: () => go("/docs") });
    }
    return out;
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((item) =>
      `${item.section} ${item.label} ${item.sub ?? ""}`.toLowerCase().includes(needle),
    );
  }, [items, search]);
