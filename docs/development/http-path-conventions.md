...
> - `packages/web/src/api/`, `packages/client/src/cloud/sdk.ts`
...
For the AgentRuntime process speaking as an agent. Parallels `/me` (user self) — both are first-person `self` namespaces, distinct from `/agents/:uuid` (resource collection). Only used by `packages/client/src/cloud/sdk.ts`.
