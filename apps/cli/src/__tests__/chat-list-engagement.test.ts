...
describe("chat list --engagement", () => {
  ...
  it("exact chat returns one item with metadata preserved when agent is a speaker and engagement matches", async () => {
    const sdk = localAgentMocks.createSdk() as MockSdk;
    ...
    expect(outputMocks.success).toHaveBeenCalledWith({
      items: [{ id: "chat-1", chatId: "chat-1", ...detail }],
      nextCursor: null,
    });
    const payload = outputMocks.success.mock.calls.at(-1)?.[0] as { items: Record<string, unknown>[] };
  ...
  it("exact chat returns empty items on an engagement mismatch", async () => {
  ...
  it("exact chat with --engagement all matches archived but never deleted", async () => {
    ...
    expect(outputMocks.success).toHaveBeenCalledWith({
      items: [{ id: "chat-1", chatId: "chat-1", ...makeChatDetail({ engagementStatus: "archived" }) }],
      nextCursor: null,
    });
