 ...
 * itself supervises the resolved CLI invocation with the daemon args (see
 ...
 * lock. In that state launchd records a pending KeepAlive spawn but may defer
 * it indefinitely, leaving the computer offline immediately after an update.
 * The already-running wrapper is not subject to that respawn gate, so it can
 * complete the handoff deterministically. Other exit codes still return to
 * launchd, preserving its ordinary crash throttling and stop semantics.
 *
 * Regenerated on every install / refresh — drift detection re-checks it, so a
 * Node or install-path change still triggers a reinstall even though the plist
...
function launchdState(): { state: ServiceState; pid?: number; detail?: string } {
  ...
  const reportedState = stateLine?.split("=")[1]?.trim();
  if (reportedState === "running") {
...
function writeLaunchdServiceFiles(): { plistPath: string; wrapperPath: string } {
  ...
  writeFileAtomically(wrapperPath, renderLaunchdWrapper(install.invocation), 0o755);

  mkdirSync(dirname(plistPath), { recursive: true });
  writeFileAtomically(plistPath, renderPlist(wrapperPath, launchdPathEnv(install.portable)), 0o644);

  return { plistPath, wrapperPath };
}

/**
 * Replace supervisor files atomically so an active launchd wrapper keeps
 * reading its complete old script while `daemon refresh-unit` installs the
 * next one. Truncating the live script in place can strand the shell between
 * loop iterations during the exact update handoff this wrapper protects.
 */
